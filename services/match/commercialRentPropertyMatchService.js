// services/match/commercialRentPropertyMatchService.js
...
class CommercialRentPropertyMatchService {
  static async getMatchedCommercialCustomerRentList(propertyId, reqUserId) {
    const matchedProperty = await CommercialRentPropertyMatch.findOne({ property_id: propertyId }).lean().exec();
    ...
    let matchedCustomerRentDetailsOther = await CommercialPropertyCustomerRent.find({ customer_id: { $in: matchedCustomerOtherIds } }).lean().exec();
...
module.exports = CommercialRentPropertyMatchService;