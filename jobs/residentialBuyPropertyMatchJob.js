...
schedule.scheduleJob('*/10 * * * * *', async () => {
  ...
  const residentialRentPropertyArr = await residentialPropertySell.find();
  ...
  for (const property of residentialRentPropertyArr) {
    ...
    const nearbyCustomersArr = await residentialCustomerBuyLocation.aggregate([
      {
        $geoNear: {
    ...
    for (const customer of nearbyCustomersArr) {
      ...
      // Check rent range
      const rentDiff = Math.abs(property.
        sell_details.expected_sell_price - customer.customer_buy_details.expected_buy_price);
      const rentRange = customer.customer_buy_details.expected_buy_price * 0.2;
      if (rentDiff <= rentRange) {
        matchScore += 10 * (1 - rentDiff / rentRange);
      }

      // // Check deposit range
      // const depositDiff = Math.abs(property.rent_details.expected_deposit - customer.customer_buy_details.expected_deposit);
      ...
      // Insert matched properties into residentialBuyCustomerMatch
      const existingCustomerMatch = await residentialBuyCustomerMatch.findOne({ customer_id: customer.customer_id });
      ...
      if (existingCustomerMatch) {
        ...
        // Update match count in ResidentialPropertyCustomerBuy schema document
        ...
        await residentialBuyCustomerMatch.create(finalCustomerObj);
    ...
    if (existingMatch) {
      ...
      // console.log('residentialBuyPropertyMatch: '+ JSON.stringify(finalObj, null, 2) );
      await residentialBuyPropertyMatch.create(finalObj);
    }

    // Update match count in residentialPropertySell schema document
