...
schedule.scheduleJob('*/10 * * * * *', async () => {
  console.log('Updating commercial rent property match...');
  const residentialRentPropertyArr = await commercialPropertyRent.find();
  ...
  for (const property of residentialRentPropertyArr) {
    ...
    const nearbyCustomersArr = await commercialCustomerRentLocation.aggregate([
      {
        $geoNear: {
    ...
    for (const customer of nearbyCustomersArr) {
      ...
      // Insert matched properties into commercialRentCustomerMatch
      const existingCustomerMatch = await commercialRentCustomerMatch.findOne({ customer_id: customer.customer_id });
      ...
      if (existingCustomerMatch) {
        ...
        await commercialRentCustomerMatch.create(finalCustomerObj);
    ...
    const existingMatch = await commercialRentPropertyMatch.findOne({ property_id: property.property_id });
    ...
    if (existingMatch) {
      ...
      // console.log('commercialRentPropertyMatch: '+ JSON.stringify(finalObj, null, 2) );
      await commercialRentPropertyMatch.create(finalObj);
    }
  ...
  console.log('Commercial rent property match done!');
});
